...
| Claude Code (architect seat, on the host) | `shell` | `host-native` | commands execute on the developer machine — the same locus as the executing seats, recorded by the seat about itself. The locus no longer keeps this seat from writing; what does is recorded on its own rows: the launcher strips its session markers from every mission it starts, and it makes no write in the checkout while a mission is in flight | `<date>` |
| Claude Cowork (consulting seat, outside the tree) | `shell` | `remote` | commands execute away from the developer machine — a property of where the session runs, recorded by that session about itself. This is the consulting seat: the locus is what makes its read neutral, since it starts nothing and merges nothing | `<date>` |
