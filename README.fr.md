...
Il est fait pour être lu. Les trois outils sont spécifiés, pas livrés. Une page l'est : le lecteur de tableau ci-dessous, que vous pouvez ouvrir tout de
suite.
...
Plusieurs agents de code IA, d'éditeurs différents et de harnais différents, qui travaillent en même temps sur un seul dépôt produisent quatre dégâts
...
connecteur GitHub du lecteur. Il ne fusionne jamais : la page est à un partage de n'importe qui, et un bouton de fusion sur une page partageable
...
[le lecteur livré, en français](https://simon-pacifae-dupont.github.io/Guildwork/fr/demo/board/reader/) ·
...
`labels.sh` depuis PowerShell). Ouvrez ensuite une issue avec le formulaire *AI mission*. Le premier contrat existe. Le lanceur qui le lira est à vous
...
mesurée le 8 septembre 2026 ; un seul humain Product Owner, un siège d'architecte en chef tenu tour à tour par
ChatGPT Work, Claude Cowork et, depuis le 18 septembre 2026, Claude Code sur la
machine de développement, et des sièges d'exécution sur Claude
...
Tous les exemples sont réécrits sur un projet fictif, **Lantern**, un tableau de bord de capteurs d'atelier relié à un contrôleur de banc, pour que les mécaniques puissent être montrées sans décrire le produit réel. Rien
...
et mener les deux premières semaines de missions à ses côtés : c'est ça, le travail. Et c'est le travail que fait l'auteur.
...
réels ; ils ont été mesurés le 9 septembre 2026, sur des jours complets uniquement.
